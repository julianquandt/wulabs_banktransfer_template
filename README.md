...
Double click on the code component and select `Begin Routine` from the top menu (see screenshot). Then, copy paste the following code into the left code field:
...
![image](https://github.com/julianquandt/wulabs_soscisurvey_banktransfer_template/assets/24586635/3748190a-e51f-481d-a04c-e16701a33450)

The right side code will be automatically generated by psychopy, and can be ignored. 
Again change the link to your own survey.
Note that the `payout` variable is automatically set to `NA` in the code above, to prevent the experiment from crashing when you are not using a payoff variable. 
This is fine, when you want to extract the payoffs from the experiment data and copy them to the payout data. 
If you want to automatically transfer the payoff amount, you can define a `payoff` variable anywhere in your experiment prior to the code above, to make sure it will be transferred to SoSciSurvey.
